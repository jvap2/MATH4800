...
class Force_Matrix():
    def __init__(self,a,b,N,t_0=0,t_m=0,M=0):
        ...
        self.N=N
        self.mid=np.asnumpy(self.mesh.midpoints())
    ...
    def Left_Ex_3(self):
        beta=.9
        f=np.zeros(self.N)
        force_int=lambda x: ((x+1)**2)*(x**beta)*((2*x/gamma(beta+2))-((x**2.63)*gamma(463/100)/(gamma(beta+3.63))))
        for i in range(self.N):
            f[i]=force_int(self.mid[i+1])-force_int(self.mid[i]) 
        return f
