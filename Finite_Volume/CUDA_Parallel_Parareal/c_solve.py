...
class Final_Solution():
    ...
    def Parareal_1(self):
        ...
        while error>tol:
            ...
            for i in range(1,m):
                if i==0:
                    b[:,:,i]=np.matmul((self.mass.Construct_Prob_1()+(1-self.theta)*self.mesh.delta_t()*self.stiff.B(self.mesh.time()[i-1])),u[:,i-1], dtype=cp.float16)+self.mesh.delta_t()*self.force.Construct()
                else:
                    b[:,:,i]=np.matmul((self.mass.Construct()+(1-self.theta)*self.mesh.delta_t()*self.stiff.B(self.mesh.time()[i-1])),u[:,i-1], dtype=cp.float16)+self.mesh.delta_t()*self.force.Construct()
                A[:,:,i]=(self.mass.Construct()-(self.theta)*self.mesh.delta_t()*self.stiff.B(t[i]))
                A_inv[:,:,i]=np.linalg.inv(A[:,:,i])
                course[:,i]=np.matmul(A_inv[:,:,i],b[:,:,i])
            ...
            error=max(np.linalg.norm(u-u_temp))
            u_temp=u
        return u
...
@cuda.jit("void(float64[:,:,:],float[:,:,:],float[:,:],float64,float64")
