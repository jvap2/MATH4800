...
import d_norms
import time
from d_norms import Left_True_Solution, Right_True_Solution
...
class Controller():
    ...
    def make_mesh(self, event):
        ...
        u_cub[1:mesh.NumofSubIntervals()+1,0]=sol.Right_Cubic_Test()
        u_lin[1:mesh.NumofSubIntervals()+1,0]=sol.Linear_Right()
        x_np=cp.asnumpy(mesh.mesh_points())
        u_true=Right_True_Solution(x_np)
        ...
        plt.grid()
        plt.show()
        norm_inf_ss=d_norms.Norm_SS_Right(x_np,u_cub)
        norm_inf_ss_lin=d_norms.Norm_SS_Right(x_np,u_lin)
