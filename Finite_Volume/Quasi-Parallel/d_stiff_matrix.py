...
class StiffMatrix():
    ...
    def Linear_Left_Deriv(self):
        col_linspace=cp.linspace(-2,1-self.N,self.N-2)
        row_linspace=cp.linspace(2,self.N-1, self.N-2)
        col_R,row_R=cp.zeros(shape=(self.N)),cp.zeros(shape=(self.N))
        col_L,row_L=cp.zeros(shape=(self.N)),cp.zeros(shape=(self.N))
        col_R[0],row_R[0]=self.gamma*(3/2)**self.beta-(1+self.gamma)*(1/2)**self.beta,self.gamma*(3/2)**self.beta-(1+self.gamma)*(1/2)**self.beta
        col_R[1],row_R[1]=(2-self.gamma)*(1/2)**self.beta-(1-self.gamma)*(3/2)**self.beta,self.gamma*((5/2)**self.beta-2*(3/2)**self.beta+(1/2)**self.beta)
        col_R[2:]=(1-self.gamma)*(2*(-col_linspace-(1/2))**self.beta-(-col_linspace-(3/2))**self.beta-(-col_linspace+(1/2))**self.beta)
        col_L[0],row_L[0]=(1-self.gamma)*(3/2)**self.beta-(2-self.gamma)*(1/2)**self.beta,(1-self.gamma)*(3/2)**self.beta-(2-self.gamma)*(1/2)**self.beta
        col_L[1],row_L[1]=(1-self.gamma)*((1/2)**self.beta+(5/2)**self.beta-2*(3/2)**self.beta),(1+self.gamma)*(1/2)**self.beta-(self.gamma)*(3/2)**self.beta
        col_L[2:]=(1-self.gamma)*((-col_linspace-(1/2))**self.beta+(-col_linspace+(3/2))**self.beta-2*(-col_linspace+(1/2))**self.beta)
        R=toeplitz(c=col_R,r=row_R)
        L=toeplitz(c=col_L,r=row_L)
        B=1/(gamma(self.beta+1)*self.h[0]**(1-self.beta))*(R+L)
        return B
stiffness=StiffMatrix(0,1,8,0,1,8,1,.3)
left=stiffness.Cubic_Left_Deriv()
print("Cubic:\n",left)
normal=stiffness.B(t=0)
print("Normal:\n",normal)
