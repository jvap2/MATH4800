...
def B_1_Cubic_Left(N,gamma,beta,h,x=0):
    ...
    B_L_Min=cp.empty(shape=(N,3))
    B_L_Plus=cp.empty(shape=(N,3))
    ...
    B=cp.matmul(K_plus_diag,(-1)*B_L_Plus)+cp.matmul(K_min_diag,(-1)*B_L_Min)
...
def B_3_Cubic_Right(N,gamma,beta,h, x=0):
    ...
    B_R_Min[N-2,2]=-B_R_Plus[N-3,2]
    B_R_Plus[N-2,2]=(-(1.5)**beta)*(6*beta**2+3*beta-4.5)
    B_R_Min[N-1,2]=-B_R_Plus[N-2,2]
...
def B_3_Cubic_Left(N,gamma,beta,h, x=0):
    ...
    B_L_Plus[N-2,2]=-B_L_Min[N-1,2]
    B_L_Min[N-2,2]=((.5)**beta)*((1/3)*(beta**2)+beta+(1/6))
    B_L_Plus[N-3,2]=-B_L_Min[N-2,2]
...
def B_2_Cubic_Left(N,gamma,beta,h, x=0):
    ...
    B_L_Plus=(-1)*toeplitz(r=col_L_plus,c=row_L_plus)
    B_L_Min=(-1)*toeplitz(r=col_L_min,c=row_L_min)
    constant=(gamma*(h**(beta-1)))/(2*math.gamma(beta+3))
    K_plus_diag=constant*diag(K_m_1[1:],k=0)
    K_min_diag=constant*diag(K_m_1[:N],k=0)
...
def B_2_Cubic_Right(N,gamma,beta,h,x=0):
    ...
    B_R_Plus=(-1)*toeplitz(r=col_R_plus,c=row_R_plus)
    B_R_Min=(-1)*toeplitz(r=col_R_min,c=row_R_min)
