...
def Mem_Eff_Lin_Op_Left(N,beta,h):
        col_linspace=cp.linspace(3,N, N-2)
        j=cp.linspace(3,N, N-2)
        j_1=cp.linspace(4,N, N-3)
        j_2=cp.linspace(5,N, N-4)
        col_min=cp.empty(shape=N+1)
        ...
        B_L_Min[4:,1]=-(-((j_1-3.5)**beta)*((1/3)*(beta**2)+beta-2*(j_1**2)+14*j_1-(143/6))+((j_1-2.5)**beta)*((4/3)*beta**2+4*beta+40*j_1-8*(j_1**2)-(142/3))+\
            ((j_1+.5)**beta)*(-3*beta**2-5*beta+8*beta*(j_1)+10*j_1-6*(j_1**2)+.5)-((j_1-1.5)**beta)*(2*beta**2+6*beta-12*j_1**2+36*j_1-23))
        B_L_Min[N,N-1]=-(((1.5)**beta)*((4/3)*beta**2+4*beta-(46/3))-((2.5)**beta)*((1/3)*beta**2+beta-(71/6)))
        B_L_Min[N-1,N-1]=((1.5)**beta)*((1/3)*beta**2+beta-(23/6))-((.5)**beta)*((4/3)*beta**2+4*beta+(2/3))
        B_L_Min[N-2,N-1]=((.5)**beta)*((1/3)*(beta**2)+beta+(1/6))
        B_L_Min[N,N-2]=-(-((1.5)**beta)*(2*(beta**2)+6*beta-23)+((2.5)**beta)*((4/3)*(beta**2)+4*beta-(142/3))-\
            ((3.5)**beta)*((1/3)*(beta**2)+beta-(143/6)))
        B_L_Min[N-1,N-2]=((.5)**beta)*(2*(beta**2)+6*beta+1)-((1.5)**beta)*((4/3)*(beta**2)+4*beta-(46/3))+\
            ((2.5)**beta)*((1/3)*(beta**2)+beta-(71/6))
        B_L_Min[N-2,N-2]=((1.5)**beta)*((1/3)*(beta**2)+beta-(23/6))-((.5)**beta)*((4/3)*(beta**2)+4*beta+(2/3))
        B_L_Min[N-3,N-2]=((.5)**beta)*((1/3)*(beta**2)+beta+(1/6))
        B_L_Min[N,N-3]=-(((3.5)**beta)*(4*(beta**2)/3+4*beta-(286/3))+((1.5)**beta)*(4*(beta**2)/3+4*beta-46/3)-((4.5**beta)*((beta**2)/3+beta-(239/6))+(2.5**beta)*(2*(beta**2)+6*beta-71)))
        B_1_P=aslinearoperator(-B_L_Min[1:,:3])
        B_1_M=aslinearoperator(B_L_Min[:-1,:3])
        B_2_P=aslinearoperator(-B_L_Min[1:,3:N-3])
        B_2_M=aslinearoperator(B_L_Min[:-1,3:N-2])
        B_3_P=aslinearoperator(-B_L_Min[1:,N-3:])
        B_3_M=aslinearoperator(B_L_Min[:-1,N-2:])
        return B_1_P, B_1_M, B_2_P,B_2_M, B_3_P, B_3_M