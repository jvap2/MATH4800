...
class StiffMatrix():
    ...
    def B(self,t):
        ...
        print(np.shape(np.diag(K_m[1:],k=0)), np.shape(self.BR()))
        B=np.ones((self.N,self.N))
        B[:]=(1/(gamma(self.beta)*self.h[0]))*(np.matmul(np.diag(K_m[1:],k=0),self.BL())+np.matmul(np.diag(K_m[:self.N+1],k=0),self.BR()))
        return B
